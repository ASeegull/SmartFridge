...
func getClientWithCookie() *http.Client {
	...
	//if err != nil {
	//	t.Errorf("could not to parse newProduct: %v", err)
	//}
	cookieJar.SetCookies(url, loginCookie)
	//if err != nil {
	//	t.Errorf("could not set loginCookie: %v", err)
	//}
...
func TestGetRecipesByProductNames(t *testing.T) {
	...
	tt := []struct {
		...
		{"no products", []string{}, http.StatusInternalServerError},
	}
...
func TestProductAdd(t *testing.T) {
	...
	tt := []struct {
		...
		{"normal product from user", &database.Product{Name: "name", ShelfLife: 2, Units: "gramm", Image: "url"}, http.StatusOK},
		{"normal product from admin", &database.Product{Name: "name", ShelfLife: 2, Units: "gramm", Image: "url"}, http.StatusOK},
	}
...
func TestProductUpdate(t *testing.T) {
	...
	tt := []struct {
		...
		{"updated product", &database.Product{Name: "name", ShelfLife: 22, Units: "gramm", Image: "URL"}, http.StatusOK},
	}
